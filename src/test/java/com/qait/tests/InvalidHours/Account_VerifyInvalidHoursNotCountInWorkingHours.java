...
public class Account_VerifyInvalidHoursNotCountInWorkingHours {

	TestSessionInitiator test;
	String date, empId,startDate,timeInfo,department;
	...
	public void Step01_insertPunchesWithIncomingEarlyHour()
	{
		...
		test.database_page.updatePunches(empId, startDate,"07:30:00");
		test.launchApplication();
		test.loginPage.login(YamlReader.getData("username"),YamlReader.getData("password"));
		test.homePage.navigateToTimeTab();
		test.timePage.clickQaitPunchesTab();
		test.timePage.sendKeysAfterQaitPunches(empId,startDate,startDate,"18:00:00");
	}
	...
	public void Step03_insertPunchesWithLeavingLateHour()
	{
		...
		test.database_page.login();
		test.database_page.gotoTable();
		test.database_page.updatePunches(empId, startDate,"13:30:00");
		test.launchApplication();
		test.loginPage.login(YamlReader.getData("username"),YamlReader.getData("password"));
		test.homePage.navigateToTimeTab();
		test.timePage.clickQaitPunchesTab();
		test.timePage.sendKeysAfterQaitPunches(empId,startDate,startDate,"20:30:00");
	}
