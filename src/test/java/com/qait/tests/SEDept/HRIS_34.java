...
public class HRIS_34 {
	...
	public void Step04_ApplyPLWPForGivenDate() {
		test.homePage.navigateToLeaveTab();
		test.leavePage.navigateToAssignLeaveTab();
		test.leavePage.selectLeaveType("Planned LWP");
		...
		test.leavePage.selectEndDate(startDate2);
		test.leavePage.clickApplyButton();
		test.pimPage.acceptAlert();
	}

	@Test(dependsOnMethods = "Step04_ApplyPLWPForGivenDate")
	public void Step05_SatAndSunMarkedSWWhenPLWPAppliedOnFridayAndMonday() {
		test.homePage.switchToDefaultContent();
		test.homePage.navigateToTimeTab();
		...
		test.timePage.verifyPLWPMessageForGivenDate(DateUtil
				.getNextDateFromGivenDateFoGivenDateModule("yyyy-MM-dd", "day",
						startDate1, 4), empId);
		test.timePage.verifyTimeValueForTheGivenDateIsEqualToTheGivenValue(
				DateUtil.getNextDateFromGivenDateFoGivenDateModule(
						"yyyy-MM-dd", "day", startDate1, 5), empId, "SW");
		test.timePage.verifyTimeValueForTheGivenDateIsEqualToTheGivenValue(
				DateUtil.getNextDateFromGivenDateFoGivenDateModule(
						"yyyy-MM-dd", "day", startDate1, 6), empId, "SW");
		test.timePage.verifyPLWPMessageForGivenDate(startDate2, empId);
	}

	@Test(dependsOnMethods = "Step05_SatAndSunMarkedSWWhenPLWPAppliedOnFridayAndMonday", alwaysRun = true)
