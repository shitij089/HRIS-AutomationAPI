...
public class HRISTPC_23 {
	...
	public void Step02_CancelAllLeaveWhichExistsInWeekToBeTested() {
		test.leavePage.cancelAllLeavesInWeekHavingWeekStartDateAsGiven(DateUtil
				.getNextDateFromGivenDateFoGivenDateModule("yyyy-MM-dd", "day",
						startDate, 5), empName, empId, monthBefore);
	...
	public void Step04_UpdateOnsitePunchesForGivenWeek() {
		...
		test.timePage.updatePunchesForWeek(DateUtil
				.getNextDateFromGivenDateFoGivenDateModule("yyyy-MM-dd", "day",
						startDate, 5), punches, "cdl");
	}

	@Test(dependsOnMethods = "Step04_UpdateOnsitePunchesForGivenWeek")
	public void Step05_VerifyHalfCompOffHoursAppliedSuccessfully() {

		test.homePage.navigateToLeaveTab();
		test.leavePage.navigateToAssignLeaveTab();
		test.leavePage.selectLeaveType("Compensatory Off");
		test.leavePage.selectEmploy(empId);
		test.leavePage
				.selectDateForApplyingCompOffDateAtParticularDate(DateUtil
						.getNextDateFromGivenDateFoGivenDateModule(
								"yyyy-MM-dd", "day", startDate, 6));
		...
		test.timePage.VerifyNoLWPIfTotalWeekHourIsGreaterThanRequiredHour(
				empId, DateUtil.getNextDateFromGivenDateFoGivenDateModule(
						"yyyy-MM-dd", "day", startDate, 7));
	}

	@Test(dependsOnMethods = "Step05_VerifyHalfCompOffHoursAppliedSuccessfully", alwaysRun = true)
